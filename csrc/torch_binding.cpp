...
std::tuple<at::Tensor, at::Tensor, at::Tensor, at::Tensor, at::Tensor> dispatch_prefill(
...
TORCH_LIBRARY_EXPAND(CONCAT(_C, _ascend), ops)
{
    ...
    ops.def(
        ...
        "str groupEp, int rank, int num_ranks, bool use_quant) -> "
        "(Tensor expandx_out, Tensor dynamic_scales_out, Tensor expand_idx_out, Tensor recv_count, Tensor recv_tokens_per_expert)");
    ...
    ops.def("combine_prefill(Tensor x, Tensor topk_idx, Tensor topk_weights, "
            "Tensor src_idx, Tensor send_head, str groupEp, int rank, int "
            "num_ranks) -> Tensor");
